...
which reached 96.07% accuracy. I also built a Gradio web app so anyone 
...
|---|---|---|---|---|
| CNN (scratch) | 85.69% | 84.14% | 78.05% | 79.72% |
| ResNet18 (my model) | 96.07% | 95.34% | 93.88% | 94.33% |
...
Just open this link and all the code is already run:
https://tinyurl.com/52m3xwzk
...
from overfitting. It got 85.69% accuracy which sounds okay but the 
recall was quite low (78.05%) meaning it missed a lot of the rarer 
disease classes.

**ResNet18 with Transfer Learning**  
This is my main model. I took a ResNet18 pre-trained on ImageNet and 
replaced the final layer with a new 38-class layer for PlantVillage. 
I used a lower learning rate (2e-4 instead of 1e-3) to avoid 
catastrophic forgetting — basically to stop the training from 
overwriting the useful features the model already learned from ImageNet. 
This worked really well and got 96.07% accuracy.
...
|---|---|---|---|
| CNN from scratch | Random | Yes | 85.69% |
| ResNet18, no augmentation, lr=0.001 | ImageNet | No | 94.05% |
| ResNet18, with augmentation, lr=0.001 | ImageNet | Yes | 93.85% |
| ResNet18, with augmentation, lr=0.0002 | ImageNet | Yes | 95.65% |

The biggest difference by far was using ImageNet pre-training. Without 
it the model struggled a lot. Dropping the learning rate gave a further 
improvement by protecting the pre-trained features during fine-tuning.
...
Try it here: https://tinyurl.com/52m3xwzk
